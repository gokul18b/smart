...
public class EvaluvationInternalAdapter extends RecyclerView.Adapter<EvaluvationInternalAdapter.Holder> {
    Context ctx;
    PojoEvaluvationInternal pojoEvaluvationInternal;

    public EvaluvationInternalAdapter(Context context, PojoEvaluvationInternal pojoEvaluvationInternal) {
        this.ctx = context;
        this.pojoEvaluvationInternal = pojoEvaluvationInternal;
    }
    ...
    public void onBindViewHolder(Holder holder, int position) {
        ...
        textSub.setText(pojoEvaluvationInternal.getData().get(position).getSub());
        holder.linearsub.addView(textSub);


        for (int i = 0; i < pojoEvaluvationInternal.getData().get(position).getResult().size(); i++) {
            ...
            if(Integer.parseInt(pojoEvaluvationInternal.getData().get(position).getResult().get(i))<40){
                textMark.setTextColor(ctx.getResources().getColor(R.color.red));
            }
            if(Integer.parseInt(pojoEvaluvationInternal.getData().get(position).getResult().get(i))!=0) {
                textMark.setText(String.valueOf(pojoEvaluvationInternal.getData().get(position).getResult().get(i)));
            }else{
                textMark.setText("-");
            }
    ...
    public int getItemCount() {
        return pojoEvaluvationInternal.getData().size();
    }
